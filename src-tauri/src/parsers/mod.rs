...
pub fn parse_dashboard_status(raw: &Value) -> Result<DashboardStatus, String> {
    ...
    Ok(DashboardStatus {
        ...
        gateway: GatewayInfo {
            mode: s(gateway, "mode", ""),
            url: s(gateway, "url", ""),
            reachable: b(gateway, "reachable", false),
            connect_latency_ms: gateway.get("connectLatencyMs").and_then(|v| v.as_i64()),
            error: gateway
                .get("error")
                .and_then(|v| v.as_str())
                .map(String::from),
        ...
        gateway_service: GatewayService {
            installed: b(gw_service, "installed", false),
            loaded: b(gw_service, "loaded", false),
            status: s(gw_runtime, "status", "unknown"),
            state: s(gw_runtime, "state", "unknown"),
            pid: gw_runtime.get("pid").and_then(|v| v.as_i64()),
            runtime_short: s(gw_service, "runtimeShort", ""),
        },
        node_service: NodeService {
            installed: b(node_service, "installed", false),
            loaded: b(node_service, "loaded", false),
            status: s(node_runtime, "status", "unknown"),
            state: s(node_runtime, "state", "unknown"),
            runtime_short: s(node_service, "runtimeShort", ""),
        },
        update: UpdateInfo {
            available: registry
                .get("latestVersion")
                ...
                .map(|v| !v.is_empty())
                .unwrap_or(false),
            latest_version: s(registry, "latestVersion", ""),
            channel: s(raw, "updateChannel", "stable"),
            install_kind: s(update, "installKind", ""),
            package_manager: s(update, "packageManager", ""),
        },
        memory: MemoryInfo {
            enabled: b(mem_plugin, "enabled", false),
            slot: s(mem_plugin, "slot", ""),
        },
        agents_total: agents
            .get("agents")
    ...
    match agents_array {
        Some(arr) => arr
            ...
            .map(|item| AgentSummary {
                id: s(item, "id", ""),
                name: s(item, "name", s(item, "id", "")),
                workspace_dir: s(item, "workspaceDir", ""),
                sessions_count: u(item, "sessionsCount", 0),
                last_updated_at: item
                    .get("lastUpdatedAt")
                    .and_then(|v| v.as_i64())
                    .unwrap_or(0),
                last_active_age_ms: item
                    .get("lastActiveAgeMs")
                    .and_then(|v| v.as_i64())
                    .unwrap_or(0),
...
pub fn parse_session_item(item: &Value) -> SessionSummary {
    let percent_used = if let (Some(used), Some(total)) = (
        item.get("totalTokens").and_then(|v| v.as_u64()),
        item.get("contextTokens").and_then(|v| v.as_u64()),
    ) {
        if total > 0 {
    ...
    SessionSummary {
        ...
        session_id: s(item, "sessionId", ""),
        kind: s(item, "kind", "direct"),
        updated_at: item.get("updatedAt").and_then(|v| v.as_i64()).unwrap_or(0),
        age_ms: item.get("ageMs").and_then(|v| v.as_i64()).unwrap_or(0),
        model: s(item, "model", "unknown"),
        model_provider: s(item, "modelProvider", ""),
        thinking_level: s(item, "thinkingLevel", ""),
        input_tokens: item.get("inputTokens").and_then(|v| v.as_u64()),
        output_tokens: item.get("outputTokens").and_then(|v| v.as_u64()),
        total_tokens: item.get("totalTokens").and_then(|v| v.as_u64()),
        context_tokens: item.get("contextTokens").and_then(|v| v.as_u64()),
        percent_used,
        remaining_tokens: item.get("remainingTokens").and_then(|v| v.as_u64()),
        total_tokens_fresh: b(item, "totalTokensFresh", false),
        system_sent: b(item, "systemSent", false),
        aborted_last_run: b(item, "abortedLastRun", false),
        flags,
    }
}
...
pub fn parse_sessions_from_list(raw: &Value) -> Vec<SessionSummary> {
    ...
    match sessions_array {
        Some(arr) => arr.iter().map(parse_session_item).collect(),
        None => vec![],
    }
}
            ...
            .map(|item| {
                ...
                CronJob {
                    ...
                    schedule: CronSchedule {
                        ...
                        expr: s(schedule, "expr", ""),
                    },
                    session_target: s(item, "sessionTarget", ""),
                    wake_mode: s(item, "wakeMode", ""),
                    ...
                    state: CronState {
                        ...
                        last_delivery_status: state
                            ...
                            .and_then(|v| v.as_str())
                            .map(String::from),
                        consecutive_errors: state
                            .get("consecutiveErrors")
                            .and_then(|v| v.as_i64())
                            .unwrap_or(0),
                        consecutive_skipped: state
                            .get("consecutiveSkipped")
                            .and_then(|v| v.as_i64())
                            .unwrap_or(0),
...
pub fn parse_skills(raw: &Value) -> Result<SkillsList, String> {
        ...
        Some(arr) => arr
            ...
            .map(|item| {
                ...
                SkillInfo {
                    ...
                    description: s(item, "description", ""),
                    emoji: s(item, "emoji", ""),
                    eligible: b(item, "eligible", false),
                    disabled: b(item, "disabled", false),
                    blocked_by_allowlist: b(item, "blockedByAllowlist", false),
                    source: s(item, "source", ""),
                    bundled: b(item, "bundled", false),
                    homepage: item
                        .get("homepage")
                        .and_then(|v| v.as_str())
                        .map(String::from),
    ...
    Ok(SkillsList {
        workspace_dir: s(raw, "workspaceDir", ""),
        managed_skills_dir: s(raw, "managedSkillsDir", ""),
        skills,
    })
}
    ...
    match models_array {
        Some(arr) => arr
            ...
            .map(|item| ModelInfo {
                key: s(item, "key", ""),
                name: s(item, "name", ""),
                input: s(item, "input", "text"),
                context_window: u(item, "contextWindow", 0),
                local: b(item, "local", false),
    ...
    match hooks_array {
        Some(arr) => arr
            ...
            .map(|item| HookInfo {
                ...
                eligible: b(item, "eligible", false),
                disabled: b(item, "disabled", false),
                enabled_by_config: b(item, "enabledByConfig", false),
                requirements_satisfied: b(item, "requirementsSatisfied", false),
                loadable: b(item, "loadable", false),
                source: s(item, "source", ""),
                events: item
                    .get("events")
                    .and_then(|v| v.as_array())
