    ...
    if config_path.exists() {
        if let Ok(content) = fs::read_to_string(&config_path) {
            if let Ok(mut config) = serde_json::from_str::<JarvisConfig>(&content) {
                ...
                config.jarvis_path =
                    format!("{}/.openclaw/agents/coderclaw/workspace/Jarvis", wsl_home());
                return config;
            }
        }
    }
...
pub fn get_config_path() -> PathBuf {
    PathBuf::from(format!("{}/{}", wsl_home(), CONFIG_DIR)).join(CONFIG_FILE)
}

/// Get the sessions directory path
pub fn get_sessions_dir() -> PathBuf {
    PathBuf::from(format!("{}/{}", wsl_home(), CONFIG_DIR)).join(SESSIONS_DIR)
}
