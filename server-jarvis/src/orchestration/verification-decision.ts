...
//
// Kept separate from LiveConductor.afterStage so the branching
// is testable without an entire pipeline. The conductor wraps
// this and threads the result into its directive stream.
...
export function decideVerificationDirective(input: {
  ...
  if (
    check.ran &&
    check.passed === true &&
    (check.tier === "existing" || check.tier === "builtin")
  ) {
    return {
      kind: "directive",
