...
export class PipelineExecutor {
   ...
   * branch (with the full pipeline as `stages`) and by a future B-02 replan
   * loop to run one segment between `conductor_replan` markers.
   * Synthesizer only runs if `"synthesizer"` is in `stages` — a non-terminal
   * segment stops right after reviewer/rewriter so a future replan loop can
